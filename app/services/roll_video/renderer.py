...
try:
    ...
    def blend_alpha_fast(source, target, alpha):
        ...
        for y in prange(h):
            for x in range(w):
                for ch in range(c):
                    a = alpha[y, x, 0]
                    result[y, x, ch] = source[y, x, ch] * a + target[y, x, ch] * (
                        1.0 - a
                    )
    ...
    logger.warning(
        "Numba未安装，将使用标准Python函数。安装Numba可大幅提升性能：pip install numba"
    )
...
class TextRenderer:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                f"加载字体失败 ('{font_path}'), 使用Pillow默认字体: {e}", exc_info=True
            )
            # 回退到Pillow内置字体
        ...
        # 将传入的参数转换为列表，以便统一处理
        font_color_list = (
            list(font_color) if not isinstance(font_color, list) else font_color.copy()
        )
        bg_color_list = (
            list(bg_color) if not isinstance(bg_color, list) else bg_color.copy()
        )
        ...
        for i in range(len(bg_color_list)):
            ...
            if (
                i == 3
                and 0 <= bg_color_list[i] <= 1
                and isinstance(bg_color_list[i], float)
            ):
        ...
        # 记录原始参数，便于调试
        logger.info(
            f"文本渲染器初始化: 字体大小={font_size}, 行间距={self.line_spacing}, 字符间距={char_spacing}"
        )
    ...
    def _calculate_text_layout(self, text: str) -> List[str]:
        ...
        estimated_chars_per_line = (
            int(self.width / avg_char_width) if avg_char_width > 0 else 1
        )
        if estimated_chars_per_line <= 0:
            estimated_chars_per_line = 1  # 避免零或负宽度
    ...
    def render_text_to_image(
        self, text: str, min_height: Optional[int] = None
        ...
        for line in lines:
            ...
            for char in line:
                draw.text(
                    (x_pos, y_position), char, font=self.font, fill=self.font_color
                )
                # 测量字符宽度
                try:
        ...
        # 记录渲染尺寸和背景色
        logger.info(
            f"Text rendered: lines={len(lines)}, text_height={text_actual_height}px, img_size={img.size}, bg_color={self.bg_color}"
        )
...
class VideoRenderer:
    ...
    def __init__(
        ...
        # 日志记录渲染配置
        logger.info(
            f"初始化视频渲染器: 宽={width}, 高={height}, FPS={fps}, 缩放={scale_factor}, 跳帧={frame_skip}, 线程数={self.max_threads}, 透明={transparent}"
        )

        if self.high_memory_mode:
            logger.info(
                f"已启用极速渲染模式: 队列容量={self.frame_queue_size}, 预填充={self.prefill_count}帧, 线程数={self.max_threads}"
            )
    ...
    def _frame_writer(self):
        ...
        try:
            ...
            logger.info(
                f"设置最大允许帧数: {max_frames_allowed}, 预期处理帧数: {expected_frames_to_process}"
            )

            # Log just before entering the loop
            logger.info("Entering main frame writing loop...")
            logger.debug(
                f"Initial state before loop: Stop event: {self._event_stop.is_set()}, Queue empty: {self._frame_queue.empty()}"
            )
            ...
            while not self._event_stop.is_set() or not self._frame_queue.empty():
                ...
                if frames_written >= max_frames_allowed:
                    logger.warning(
                        f"已达到最大允许帧数 {max_frames_allowed}，强制停止处理"
                    )
                    self._event_stop.set()
                    break
                ...
                if loop_counter % 1000 == 0:  # 每1000次迭代记录一次日志
                    logger.debug(
                        f"帧处理循环: {loop_counter}, 已写入: {frames_written}/{expected_frames_to_process}"
                    )
                ...
                try:
                    ...
                    logger.debug(
                        f"Loop {loop_counter}: Attempting to get frame from queue. Stop event: {self._event_stop.is_set()}, Queue empty: {self._frame_queue.empty()}"
                    )  # Reverted to DEBUG
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Loop {loop_counter}: Got frame data from queue."
                        )  # Reverted to DEBUG
                    except queue.Empty:
                        ...
                        if self._event_stop.is_set() and self._frame_queue.empty():
                            ...
                            logger.info(
                                f"停止信号已设置且队列为空，退出循环 (已处理 {frames_written} 帧)"
                            )
                            break  # Exit loop condition met
                        continue  # Continue loop to check stop event again or wait for frames
                    except Exception as e:
                        logger.error(
                            f"Loop {loop_counter}: Unexpected error getting from queue: {str(e)}",
                            exc_info=True,
                        )
                        self._error.value = 1
                    ...
                    if frame_data is None:
                        logger.warning(
                            f"Loop {loop_counter}: Got None frame data, skipping."
                        )
                        continue
                    ...
                    if max_frames_allowed > 0 and frame_idx >= max_frames_allowed:
                        if frame_idx % 100 == 0:  # 降低日志频率
                            logger.warning(
                                f"帧索引 {frame_idx} 超出最大允许值 {max_frames_allowed}，跳过此帧"
                            )
                        continue  # 超出最大帧数的帧直接跳过

                    # 更新最后处理的帧索引
                    with self._last_frame_lock:
                        self._last_frame_processed = max(
                            self._last_frame_processed, frame_idx
                        )
                    ...
                    if len(frame_buffer) >= buffer_size or (
                        ...
                        if ffmpeg_process and ffmpeg_process.poll() is not None:
                            ...
                            logger.error(
                                f"FFmpeg进程已异常终止，错误信息: {error_output}"
                            )
                        ...
                        for idx, frame_data in frame_buffer:
                            ...
                            if frames_written >= max_frames_allowed:
                                logger.info(
                                    f"已写入最大允许帧数 {max_frames_allowed}，停止写入"
                                )
                                self._event_stop.set()
                                break
                            ...
                            try:
                                if frame_data is not None:
                                    ...
                                    # 如果是NumPy数组，需要确保内存连续
                                    elif hasattr(frame_data, "copy") and hasattr(
                                        frame_data, "tobytes"
                                    ):
                                        frame_bytes = frame_data.copy(
                                            order="C"
                                        ).tobytes()
                                    # 其他情况下跳过这一帧
                                    else:
                                        logger.warning(
                                            f"未知帧数据类型: {type(frame_data)}"
                                        )
                                        continue

                                    # 写入FFmpeg进程
                                    if (
                                        ffmpeg_process and ffmpeg_process.poll() is None
                                    ):  # 确保进程仍在运行
                                        ...
                                        # 更新进度条
                                        frames_to_update = (
                                            frames_written - last_updated_frame
                                        )
                                        if frames_to_update > 0:
                                            if (
                                                ...
                                                try:
                                                    ...
                                                    logger.debug(
                                                        f"Loop {loop_counter}: Updated progress bar by {frames_to_update}"
                                                    )
                                                except Exception as pb_e:
                                                    logger.error(
                                                        f"Loop {loop_counter}: Error updating progress bar: {pb_e}"
                                                    )
                                ...
                                if consecutive_errors < max_consecutive_errors:
                                    ...
                                    try:
                                        logger.warning(
                                            f"连续{max_consecutive_errors}次重启FFmpeg失败，尝试简单模式..."
                                        )
                                        ...
                                        logger.info(
                                            f"尝试最简单的命令: {' '.join(simple_command)}"
                                        )
                    ...
                    logger.debug(
                        f"Loop {loop_counter}: End of iteration. Stop event: {self._event_stop.is_set()}, Queue empty: {self._frame_queue.empty()}"
                    )  # Reverted to DEBUG

                except Exception as loop_e:
                    # Catch any unexpected error within the main try block of the loop
                    logger.error(
                        f"Loop {loop_counter}: Unexpected error in main loop body: {str(loop_e)}",
                        exc_info=True,
                    )
            ...
            logger.info(
                f"帧写入主循环已结束. Final loop count: {loop_counter}. Stop event: {self._event_stop.is_set()}, Queue empty: {self._frame_queue.empty()}"
            )
            ...
            try:
                ...
                if frames_written > 0:
                    ...
                    logger.info(
                        f"帧写入完成: 已写入 {frames_written} 帧, 耗时 {total_time:.2f} 秒, 平均速度 {frames_written/total_time:.2f} 帧/秒"
                    )
                    if skipped_frames > 0:
                ...
                try:
                    if ffmpeg_process and ffmpeg_process.poll() is None:
                        ...
                        try:
                            ...
                            logger.info(
                                f"GPU类型检测: {gpu_info} - 高端GPU: {is_high_end_gpu}"
                            )
                        except Exception as e:
                        ...
                        try:
                            ...
                            logger.error(
                                f"创建 FFmpeg 等待进度条失败: {str(e)}", exc_info=True
                            )
                        wait_complete = False
                        ...
                        for i in range(progress_points):
                            if ffmpeg_process.poll() is not None:
                                ...
                                if wait_bar:
                                    ...
                                    wait_bar.refresh()
                                logger.info(
                                    f"FFmpeg进程在等待{i+1}/{progress_points}段后自行结束"
                                )
                                break
                            elapsed_wait = time.time() - ffmpeg_start_wait
                            percent_done = min(
                                100, int((i + 1) / progress_points * 100)
                            )
                        ...
                        if not wait_complete:
                            logger.warning(
                                f"FFmpeg进程超时未退出，强制终止 (已等待{int(time.time()-ffmpeg_start_wait)}秒)"
                            )
                            ffmpeg_process.terminate()
                            ...
                            while (
                                ...
                                and time.time() - termination_start
                                < termination_timeout
                            ):
                                time.sleep(0.5)
                            if ffmpeg_process.poll() is None:
                            ...
                            if (
                                ...
                                and os.path.getsize(self.output_path) > 1000000
                            ):
                                logger.info(
                                    f"尽管超时，但输出文件已生成，大小: {os.path.getsize(self.output_path)/1024/1024:.2f}MB"
                                )
                            else:
                            ...
                            if return_code != 0:
                                ...
                                logger.error(
                                    f"FFmpeg进程异常退出，返回值: {return_code}, 错误信息: {stderr_output}"
                                )
                                if self._error.value == 0:
                                    self._error.value = (
    ...
    def _should_stop_scrolling(self, frame_index):
        ...
        if frame_index >= max_scroll_frames:
            logger.info(
                f"帧 {frame_index} 已达到最大滚动帧数 {max_scroll_frames}，应停止滚动"
            )
            return True
    ...
    def _prepare_ffmpeg_command(self):
        ...
        if (
            output_path.endswith(".tmp")
            or os.path.splitext(output_path)[1] != output_ext
        ):
        ...
        command = [
            ...
            "rawvideo",  # 输入编解码器
            "-s",
            f"{self.width}x{self.height}",  # 视频尺寸
            "-pix_fmt",
            "rgba" if self.transparent else "rgb24",  # 像素格式
            "-r",
            str(self.fps),  # 帧率
            "-i",
            "pipe:",  # 从管道读取
        ...
        if hasattr(self, "total_frames") and self.total_frames > 0:
            ...
            command.extend(
                [
                    "-frames:v",
                    str(self.total_frames),  # 明确帧数
                    "-t",
                    str(exact_duration),  # 明确时长
                ]
            )
            logger.info(
                f"强制设置非循环参数: 总帧数={self.total_frames}, 时长={exact_duration:.2f}秒"
            )
        ...
        try:
            ...
            # Linux平台
                ...
                try:
                    ...
                    try:
                        output = subprocess.check_output(
                            "lspci | grep -i nvidia", shell=True, text=True
                        )
                        if output:
                            # 需要进一步确认NVENC可用性
                            try:
            ...
            # macOS平台
                ...
                # 使用system_profiler
                output = subprocess.check_output(
                    "system_profiler SPDisplaysDataType", shell=True, text=True
                )
    ...
    def render_frames(self, total_frames, frame_generator):
        ...
        try:
            ...
            logger.info(
                f"初始内存使用: {initial_memory / 1024 / 1024:.2f} MB，允许最大内存使用: {memory_limit / 1024 / 1024 / 1024:.0f}GB"
            )
            ...
            try:
                ...
                logger.error(
                    f"启动帧写入线程 (_frame_writer) 时出错: {start_e}", exc_info=True
                )
                self._error.value = 1
            ...
            if writer_thread:
                logger.info(
                    f"主线程: 准备等待写入线程 (_frame_writer) 结束 (timeout=None - wait indefinitely)..."
                )
            ...
            if self._error.value != 0:
                ...
                if self.error_callback:
                    self.error_callback(
                        f"渲染过程中发生错误，错误码: {self._error.value}"
                    )
            ...
            logger.info(
                f"渲染完成。内存使用: {final_memory / 1024 / 1024:.2f} MB (增长: {memory_growth / 1024 / 1024:.2f} MB, 峰值: {peak_memory / 1024 / 1024:.2f} MB)"
            )
            ...
            logger.error(
                f"渲染视频时发生意外错误 (render_frames): {str(e)}", exc_info=True
            )
            if self.error_callback:
                ...
                for proc in current_process.children(recursive=True):
                    try:
                        if "ffmpeg" in proc.name().lower():
                            logger.info(
                                f"正在终止子进程: {proc.name()} (PID: {proc.pid})"
                            )
                            proc.terminate()
                            # 给进程一些时间正常退出
                            proc.wait(timeout=2)
            ...
            try:
                if "old_threshold" in locals():
                    ...
                    if current_threshold != old_threshold:
                        logger.info(
                            f"在 render_frames finally 中恢复GC阈值: {current_threshold} -> {old_threshold}"
                        )
                        gc.set_threshold(*old_threshold)
            except NameError:
    ...
    def calculate_total_frames(self, text_height, scroll_speed):
        """计算视频需要的总帧数，确保文本完全滚出屏幕"""
        # 确保滚动速度至少为0.5像素/帧
        actual_scroll_speed = max(0.5, scroll_speed)
        logger.info(
            f"使用实际滚动速度: {actual_scroll_speed}px/帧 (原始: {scroll_speed})"
        )

        # 计算文本完全滚出屏幕所需的总滚动距离
        # 总滚动距离 = 文本自身高度 + 视频高度 (确保底部完全移出)
        # 注意：这里使用 self.height (渲染器实例的视频高度)
        scroll_distance_needed = text_height + self.height
        logger.info(
            f"【帧数计算】文本高度: {text_height}px, 视频高度: {self.height}px, 总滚动距离: {scroll_distance_needed}px"
        )

        # 计算完全滚出所需的帧数
        # 总帧数 = 总滚动距离 / 滚动速度 (向上取整)
        total_frames = int(np.ceil(scroll_distance_needed / actual_scroll_speed))

        logger.info(f"【帧数计算】滚动速度: {actual_scroll_speed}px/帧")
        logger.info(f"【帧数计算】计算得到的总帧数: {total_frames}帧")
        logger.info(
            f"【帧数计算】预计视频时长: {total_frames/self.fps:.2f}秒，帧率: {self.fps}fps"
        )
